...
import './actual-achievement-item.scss';

const ActualAchievementItem = ({ actualAchievementItem: { description, status, image } }) => {
  return (
    <li className='actual-achievement-item'>
      <button className={`actual-achievement-item__button ${status === 'SUCCESS' ? 'achievement-item__button--done' : ''}`} aria-label="Center Align">
